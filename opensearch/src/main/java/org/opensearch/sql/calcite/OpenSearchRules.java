 ...
 * {@link OpenSearchRel#CONVENTION OpenSearch}
 * calling convention.
 */
class OpenSearchRules {
  static final RelOptRule[] RULES = {
      OpenSearchSortRule.INSTANCE,
      OpenSearchFilterRule.INSTANCE,
      OpenSearchProjectRule.INSTANCE,
      OpenSearchAggregateRule.INSTANCE
  };
  ...
  static List<String> OpenSearchFieldNames(final RelDataType rowType) {
  ...
  /**
   * Translator from {@link RexNode} to strings in OpenSearch's expression
   * language.
   */
  static class RexToOpenSearchTranslator extends RexVisitorImpl<String> {
    private final JavaTypeFactory typeFactory;
    private final List<String> inFields;

    RexToOpenSearchTranslator(JavaTypeFactory typeFactory, List<String> inFields) {
      super(true);
    ...
    @Override public String visitCall(RexCall call) {
      ...
      throw new IllegalArgumentException("Translation of " + call
          + " is not supported by OpenSearchProject");
    }
  }

  /**
   * Base class for planner rules that convert a relational expression to
   * OpenSearch calling convention.
   */
  abstract static class OpenSearchConverterRule extends ConverterRule {
    protected OpenSearchConverterRule(Config config) {
      super(config);
    }
  }
   ...
   * {@link OpenSearchSort}.
   */
  private static class OpenSearchSortRule extends OpenSearchConverterRule {
    private static final OpenSearchSortRule INSTANCE = Config.INSTANCE
        .withConversion(Sort.class, Convention.NONE,
            OpenSearchRel.CONVENTION, "OpenSearchSortRule")
        .withRuleFactory(OpenSearchSortRule::new)
        .toRule(OpenSearchSortRule.class);
    ...
    @Override public RelNode convert(RelNode relNode) {
      ...
      return new OpenSearchSort(relNode.getCluster(), traitSet,
   ...
   * {@link OpenSearchFilter}.
   */
  private static class OpenSearchFilterRule extends OpenSearchConverterRule {
    private static final OpenSearchFilterRule INSTANCE = Config.INSTANCE
        .withConversion(LogicalFilter.class, Convention.NONE,
            OpenSearchRel.CONVENTION, "OpenSearchFilterRule")
        .withRuleFactory(OpenSearchFilterRule::new)
        .toRule(OpenSearchFilterRule.class);
    ...
    @Override public RelNode convert(RelNode relNode) {
      ...
      return new OpenSearchFilter(relNode.getCluster(), traitSet,
   ...
   * to an {@link OpenSearchAggregate}.
   */
  private static class OpenSearchAggregateRule extends OpenSearchConverterRule {
    private static final RelOptRule INSTANCE = Config.INSTANCE
        .withConversion(LogicalAggregate.class, Convention.NONE,
            OpenSearchRel.CONVENTION, "OpenSearchAggregateRule")
        .withRuleFactory(OpenSearchAggregateRule::new)
        .toRule(OpenSearchAggregateRule.class);
   ...
   * to an {@link OpenSearchProject}.
   */
  private static class OpenSearchProjectRule extends OpenSearchConverterRule {
    private static final OpenSearchProjectRule INSTANCE = Config.INSTANCE
        .withConversion(LogicalProject.class, Convention.NONE,
            OpenSearchRel.CONVENTION, "OpenSearchProjectRule")
        .withRuleFactory(OpenSearchProjectRule::new)
        .toRule(OpenSearchProjectRule.class);
    ...
    @Override public RelNode convert(RelNode relNode) {
      ...
      return new OpenSearchProject(project.getCluster(), traitSet,
