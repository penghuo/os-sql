 ...
 *
 * <p>The store owns the public lifecycle contract: authorization, lease renewal, immutable sequence
 * snapshots, long-poll waiters, final paging state, cancellation, and expiration.
 */
class PPLAsyncQueryJobService {
  ...
  static final TimeValue DEFAULT_WAIT_FOR_COMPLETION = TimeValue.timeValueSeconds(1);
  static final TimeValue MAX_WAIT_FOR_COMPLETION = TimeValue.timeValueSeconds(30);
  ...
  PPLAsyncQueryJobService(Supplier<String> ownerNodeIdSupplier, ThreadPool threadPool) {
    this(
        ownerNodeIdSupplier,
  ...
  String create(String user, TimeValue keepAlive, CancellableTask task) {
    reapExpired();
  ...
  Snapshot get(String id, String user, TimeValue keepAlive) {
  ...
  void awaitCompletion(
      String id, String user, TimeValue waitTimeout, ActionListener<Snapshot> listener) {
    validateWaitTimeout(waitTimeout);
    Job job = findAuthorized(id, user);
    WaitRegistration registration =
        job.registerWaiter(currentTimeMillis.getAsLong(), null, -1L, true, waitTimeout, listener);
    finishRegistration(job, registration);
  }

  Snapshot removeSuccessfulFastPath(String id, String user) {
  ...
  Snapshot cancelAndRemove(String id, String user) {
    PPLAsyncQueryJobId jobId = localId(id);
    Job job = jobs.get(jobId.contextId());
    if (job == null) {
      throw unknown(id);
  ...
  private Job findAuthorized(String id, String user) {
    PPLAsyncQueryJobId jobId = localId(id);
    Job job = jobs.get(jobId.contextId());
    if (job == null) {
      throw unknown(id);
  ...
  private static final class Waiter {
    private final long waitForSequence;
    private final boolean completionOnly;
    private final ActionListener<Snapshot> listener;
    private final AtomicBoolean completed = new AtomicBoolean();
    private volatile TimeoutHandle timeout;

    private Waiter(
        long waitForSequence, boolean completionOnly, ActionListener<Snapshot> listener) {
      this.waitForSequence = waitForSequence;
    ...
    private boolean shouldComplete(Snapshot snapshot) {
      return snapshot.status() != Status.RUNNING
          || (!completionOnly && snapshot.sequence() > waitForSequence);
    }
  ...
  private static final class Job {
    ...
    private QueryProgress progress = QueryProgress.UNKNOWN;
    ...
    private synchronized void authorize(String requestingUser) {
      if (!Objects.equals(user, requestingUser)) {
        throw new OpenSearchSecurityException(
            "User is not authorized to access PPL job id [" + id + "]");
      }
    }
    ...
    private synchronized Publication progress(QueryProgress progress) {
      if (status != Status.RUNNING || Objects.equals(this.progress, progress)) {
        return null;
      }
    ...
    private synchronized Publication complete(QueryResponse response, long now) {
      ...
      this.response = response;
      progress = new QueryProgress(1D, progress.shardsTotal(), progress.shardsCompleted());
    ...
    private synchronized WaitRegistration registerWaiter(
        long now,
      ...
      if (waitTimeout.millis() == 0
          || snapshot.status() != Status.RUNNING
          || (!completionOnly && snapshot.sequence() > waitForSequence)) {
        return WaitRegistration.immediate(snapshot, listener);
      }
      Waiter waiter = new Waiter(waitForSequence, completionOnly, listener);
      waiters.add(waiter);
