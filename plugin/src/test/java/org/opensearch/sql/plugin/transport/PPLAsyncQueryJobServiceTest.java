...
public class PPLAsyncQueryJobServiceTest {
  ...
  public void append_job_publishes_progress_partial_rows_and_final_rows() {
    ...
    service.progress(id, new QueryProgress(0.5D, 4, 2));
    ...
    assertEquals(3, partial.sequence());
    ...
    assertEquals(4, completed.sequence());
  ...
  public void replace_job_replaces_running_snapshot_and_accepts_final_rows() {
    ...
    service.progress(id, new QueryProgress(-1D, 3, 1));
    ...
    assertEquals(1, running.progress().shardsCompleted());
  ...
  @Test
  public void partial_rows_and_result_progress_share_one_sequence() {
    String id = createJob();
    service.classify(id, UpdateMode.APPEND);

    service.partial(id, response(1), new QueryProgress(0.25D, -1, -1));
    ...
    assertEquals(new QueryProgress(0.25D, -1, -1), snapshot.progress());
  }
  ...
  @Test
  public void rejects_another_user() {
    String id = createJob();
    assertThrows(OpenSearchSecurityException.class, () -> service.get(id, "bob", null));
  }
  ...
  @Test
  public void long_poll_completes_on_the_next_sequence() {
    ...
    longPollService.classify(id, UpdateMode.APPEND);

    assertEquals(1, result.get().sequence());
    assertEquals(UpdateMode.APPEND, result.get().updateMode());
  }

  @Test
  public void long_poll_timeout_returns_the_current_snapshot() {
    AtomicReference<Runnable> timeoutTask = new AtomicReference<>();
    PPLAsyncQueryJobService longPollService =
        new PPLAsyncQueryJobService(
            OWNER,
    ...
    assertEquals(0, result.get().sequence());
