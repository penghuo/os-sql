...
public class SparkQueryDispatcher {
  ...
  public JSONObject getQueryResponse(AsyncQueryJobMetadata asyncQueryJobMetadata) {
    ...
    } else if (isIndexDMLQuery(asyncQueryJobMetadata.getJobId())) {
      return new IndexDMLHandler(emrServerlessClient, dataSourceService,
          dataSourceUserAuthorizationHelper, jobExecutionResponseReader,
          flintIndexMetadataReader, client, stateStore).getQueryResponse(asyncQueryJobMetadata);
    } else {
  ...
  public String cancelJob(AsyncQueryJobMetadata asyncQueryJobMetadata) {
    if (asyncQueryJobMetadata.getSessionId() != null) {
      ...
      return new IndexDMLHandler(emrServerlessClient, dataSourceService,
          dataSourceUserAuthorizationHelper, jobExecutionResponseReader,
          flintIndexMetadataReader, client, stateStore).cancelJob(asyncQueryJobMetadata);
    } else {
      return new BatchQueryHandler(emrServerlessClient, jobExecutionResponseReader)
          .cancelJob(asyncQueryJobMetadata);
    }
  ...
  private DispatchQueryResponse handleSQLQuery(DispatchQueryRequest dispatchQueryRequest) {
    if (SQLQueryUtils.isIndexQuery(dispatchQueryRequest.getQuery())) {
      ...
      if (indexDetails.isDropIndex()) {
        return new IndexDMLHandler(emrServerlessClient, dataSourceService,
            dataSourceUserAuthorizationHelper, jobExecutionResponseReader,
            flintIndexMetadataReader, client, stateStore).handle(dispatchQueryRequest, indexDetails);
      } else {
  ...
  private DispatchQueryResponse handleNonIndexQuery(DispatchQueryRequest dispatchQueryRequest) {
    ...
    if (sessionManager.isEnabled()) {
      ...
      String jobId = emrServerlessClient.startJobRun(startJobRequest);
      return new DispatchQueryResponse(
          queryId, jobId, dataSourceMetadata.getResultIndex(), null);
    }
  }
