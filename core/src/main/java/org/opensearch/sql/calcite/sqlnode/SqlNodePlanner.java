...
public final class SqlNodePlanner {
  ...
  /**
   * Build a row-type oracle that validates a probe SqlNode in the same prepare context and returns
   * its row type. Used by callers that need schema introspection beyond {@link #tableFields()} or
   * {@link #tableRowType()} — e.g. visitors driving expression-shape decisions off mid-pipeline
   * SqlNode probes.
   */
  ...
  private SqlOperatorTable buildOperatorTable() {
    ...
    tables.add(new SingleOperatorTable(PermissiveIsEmpty.INSTANCE));
  ...
  private RelNode convert(
    ...
    // implicit SELECT * that PPLToSqlNodeVisitor.visitFilter emits to keep adjacent filter pipes
    // as separate Filter rels (matching v2's plan shape). The Calcite optimizer doesn't strip
    // these when trimUnusedFields is off.
   ...
   * is a LogicalFilter or LogicalUnion — these come from the implicit SELECT * that
   * PPLToSqlNodeVisitor.visitFilter emits to keep adjacent filter pipes as separate Filter rels, or
   * when visitUnion/visitAppend produce a SqlSelect over the union. The Calcite optimizer doesn't
   * strip them because trimUnusedFields is off.
   */
  ...
  /**
   * Collapse the SPAN-pre-wrap pattern emitted by visitAggregation. The visitor wraps the source
  ...
  /**
   * Rewrite RexCalls on {@link PermissiveIsEmpty#INSTANCE} to the standard {@link
   * SqlStdOperatorTable#IS_EMPTY} postfix operator. The permissive variant is used at the
    ...
    org.apache.calcite.rex.RexShuttle rexShuttle =
        new org.apache.calcite.rex.RexShuttle() {
          ...
          public org.apache.calcite.rex.RexNode visitCall(org.apache.calcite.rex.RexCall call) {
            org.apache.calcite.rex.RexCall visited =
                (org.apache.calcite.rex.RexCall) super.visitCall(call);
            if (visited.getOperator() == PermissiveIsEmpty.INSTANCE) {
              return rel.getCluster()
                  .getRexBuilder()
