...
| 119 | CalciteStreamstatsCommandIT | ⚠️ | 44/47 pass after Track E9 partial fix: wrap the no-group streamstats path in `__stream_seq__ + Sort($seq)` so the OVER window evaluation survives the round-trip (ANSI SQL leaves OVER() without ORDER BY undefined; the upstream `sort age` was being dropped during round-trip's window evaluation). Adding the seq column + outer sort survives via `withRemoveSortInSubQuery(false)`. Plus `withRemoveSortInSubQuery(false)` (see #38). 3 fails remaining: `testMultipleStreamstatsWithEval`, `testMultipleStreamstatsWithWindow`, `testStreamstatsResetWithNullBucket` — these involve LEFT-Correlate joinType drift on the reset path or compound window+self-join shapes. Deferred. |
