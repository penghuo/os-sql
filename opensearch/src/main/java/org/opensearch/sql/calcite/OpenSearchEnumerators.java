 ...
 */
class OpenSearchEnumerators {

  private OpenSearchEnumerators() {}

  @SuppressWarnings("unused")
  private static Function1<OpenSearchJson.SearchHit, Map> mapGetter() {
    return OpenSearchJson.SearchHit::sourceOrFields;
  }

  private static Function1<OpenSearchJson.SearchHit, Object> singletonGetter(
    ...
    return hit -> {
      ...
      final Object value;
      if (OpenSearchConstants.ID.equals(key)
          || OpenSearchConstants.ID.equals(mapping.getOrDefault(fieldName, fieldName))) {
   ...
   */
  private static Function1<OpenSearchJson.SearchHit, Object[]> listGetter(
    ...
    return hit -> {
      ...
      for (int i = 0; i < fields.size(); i++) {
        ...
        final Object value;
        if (OpenSearchConstants.ID.equals(key)
            || OpenSearchConstants.ID.equals(mapping.get(field.getKey()))
            || OpenSearchConstants.ID.equals(field.getKey())) {
          // is the original projection on _id field?
          value = hit.id();
        } else {
          value = hit.valueOrNull(key);
  ...
  static Function1<OpenSearchJson.SearchHit, Object> getter(
