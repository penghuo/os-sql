...
- `language-grammar/`: Shared grammar utilities across modules.
...
- `./gradlew :<module>:compileJava`: Quick compile check for a single module (e.g., `:core:compileJava`).
- `./gradlew generateGrammarSource`: Regenerate ANTLR parsers from `.g4` grammars.
...
- ANTLR grammars (`.g4`) live in `<module>/src/main/antlr/`. Never edit files under `**/antlr/parser/**` -- those are generated.
