...
public abstract class FlintIndexOp {
  ...
  public void apply(FlintIndexMetadata metadata) {
    ...
    Optional<String> flintIndexStateDocId = metadata.getFlintIndexStateDocId();
    if (flintIndexStateDocId.isEmpty()) {
      //take action without occ.
      FlintIndexStateModel fakeModel =
          new FlintIndexStateModel(FlintIndexState.REFRESHING, metadata.getAppId(),
              metadata.getJobId(), datasourceName, System.currentTimeMillis(), "",
              SequenceNumbers.UNASSIGNED_SEQ_NO, SequenceNumbers.UNASSIGNED_PRIMARY_TERM);
      runOp(fakeModel);
    } else {
      Optional<FlintIndexStateModel> flintIndexOptional = getFlintIndexState(stateStore,
              datasourceName).apply(flintIndexStateDocId.get());
      if (flintIndexOptional.isEmpty()) {
        String errorMsg =
            String.format(Locale.ROOT, "no state found. docId: %s", flintIndexStateDocId.get());
        LOG.error(errorMsg);
      ...
      try {
        flintIndex = updateFlintIndexState(stateStore, datasourceName).apply(flintIndex,
            transitioningState());
      } catch (Exception e) {
        String errorMsg =
            String.format(Locale.ROOT, "begin failed. target transitioning state: [%s]",
                transitioningState);
      ...
      try {
        ...
        String errorMsg =
            String.format(Locale.ROOT, "commit operation failed. target stable state: [%s]",
                stableState);
        LOG.error(errorMsg, e);
        throw new IllegalStateException(errorMsg, e);
      }
