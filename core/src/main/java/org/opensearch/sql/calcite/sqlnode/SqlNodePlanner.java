...
public final class SqlNodePlanner {
  ...
  private RelNode convert(
    ...
    // GROUP BY context and assigns `$fN`. Mirror v2's emission shape.
    ...
    // has no group-by AND all aggs are count-family (or single computed eval-arg). v2's RelBuilder
    // reserves position 0 for the agg result; SqlValidator names projection cols from position 0.
    ...
    // Pull a Project-on-Sort-on-Project sub-pattern up: `Sort -> Project -> Sort -> X` matches
    // the URL fetch_size + user `head N | fields col` shape; v2 emits Project BETWEEN the two
    // Sorts (`Sort -> Sort -> Project` becomes `Sort -> Project -> Sort`).
    rel = liftProjectBetweenNestedSorts(rel);
    // Push a passthrough Project sitting BETWEEN nested Sorts down below the inner Sort.
    // `Sort(outer) -> Project(passthrough,reorder) -> Sort(fetch-only) -> X` becomes
    // `Sort(outer-reindexed) -> Sort(fetch-only) -> Project -> X`. Matches v2's emission shape
    // for `... | stats ... | head N | fields a, b` where the user-fields Project goes BELOW
    // the inner head-Sort and BELOW the URL-fetch_size outer Sort.
    rel = lowerProjectBetweenNestedSorts(rel);
    ...
    // Add $cor0 to variablesSet on Project/Filter nodes that contain a RexSubQuery operand.
    // v2 always tracks `$cor0` in the parent's variablesSet when a subquery is present (even
    // for uncorrelated subqueries); SqlNode→RelNode conversion only sets it for actually-
    // correlated subqueries. Match v2's emission shape.
    rel = addCor0ToSubQueryParents(rel);
  ...
  /**
   * Match `Sort -> Sort -> Project(passthrough) -> X` and transform to `Sort -> Project -> Sort ->
   * X` (lift the Project between the nested Sorts). This is the URL fetch_size + user `head N |
   * fields col` shape — v2 puts the implicit fields-Project between the URL Sort and the user
   * `head` Sort.
   *
   * <p>Re-indexes the inner Sort's collation field positions through the Project's mapping so the
   * moved Sort references the original input columns.
   */
        ...
        new org.apache.calcite.rel.RelHomogeneousShuttle() {
          ...
          public RelNode visit(RelNode other) {
            ...
            if (visited instanceof org.apache.calcite.rel.logical.LogicalSort outerSort
              ...
              // Build mapping from projected col index -> input col index.
              java.util.List<org.apache.calcite.rex.RexNode> exprs = proj.getProjects();
              int[] projToInput = new int[exprs.size()];
              for (int i = 0; i < exprs.size(); i++) {
                projToInput[i] = ((org.apache.calcite.rex.RexInputRef) exprs.get(i)).getIndex();
              }
              // Re-build inner Sort over Project's input (the original Scan/Filter).
              org.apache.calcite.rel.logical.LogicalSort newInner =
                  org.apache.calcite.rel.logical.LogicalSort.create(
                      proj.getInput(), innerSort.getCollation(), innerSort.offset, innerSort.fetch);
              // Project sits on top of the inner Sort.
              org.apache.calcite.rel.logical.LogicalProject newProj =
                  org.apache.calcite.rel.logical.LogicalProject.create(
                      newInner,
                      proj.getHints(),
                      ...
                      proj.getRowType().getFieldNames(),
                      proj.getVariablesSet());
              // Re-index the OUTER Sort's collation through the Project (no change since outer
              // already references projected positions and Project hasn't moved).
              org.apache.calcite.rel.logical.LogicalSort newOuter =
                  org.apache.calcite.rel.logical.LogicalSort.create(
  ...
  /**
   * Match `Sort(outer) -> Project(passthrough) -> Sort(inner FETCH-only) -> X` and transform to
   * `Sort(outer-reindexed) -> Sort(inner) -> Project -> X`. This is the `... | stats ... | head N |
   * fields a, b` shape where v2 emits the user-fields Project BELOW the inner head-Sort and BELOW
   * the URL-fetch_size outer Sort.
   *
   * <p>The outer Sort's collation/offset/fetch reference projected positions; after pushing Project
   * below the inner Sort, the outer Sort references input positions, so we re-index the outer
   * collation field positions through the Project's mapping.
   */
  private RelNode lowerProjectBetweenNestedSorts(RelNode rel) {
    org.apache.calcite.rel.RelShuttle shuttle =
        new org.apache.calcite.rel.RelHomogeneousShuttle() {
          @Override
          public RelNode visit(RelNode other) {
            ...
            if (visited instanceof org.apache.calcite.rel.logical.LogicalSort outerSort
                && outerSort.getInput()
                    instanceof org.apache.calcite.rel.logical.LogicalProject proj
                && proj.getInput() instanceof org.apache.calcite.rel.logical.LogicalSort innerSort
                && isPassthroughProject(proj)
                && innerSort.fetch != null
                && innerSort.getCollation().getFieldCollations().isEmpty()
                // Only fire when the inner Sort's input is an Aggregate. This is the
                // `... | stats ... | head N | fields a, b` shape where v2 emits the user-fields
                // Project BELOW the inner head-Sort. Skip when inner Sort feeds Scan/Filter — that
                // case is the URL-fetch_size + user `head N | fields col` shape handled by
                // liftProjectBetweenNestedSorts (which would be reversed if we fire here).
                && innerSort.getInput() instanceof org.apache.calcite.rel.core.Aggregate) {
              // Build mapping projected col index -> input col index.
              java.util.List<org.apache.calcite.rex.RexNode> exprs = proj.getProjects();
              int[] projToInput = new int[exprs.size()];
              for (int i = 0; i < exprs.size(); i++) {
                projToInput[i] = ((org.apache.calcite.rex.RexInputRef) exprs.get(i)).getIndex();
              }
              // Re-index outer Sort's collation field positions through the mapping.
              org.apache.calcite.rel.RelCollation oldCol = outerSort.getCollation();
              java.util.List<org.apache.calcite.rel.RelFieldCollation> newFields =
                  new java.util.ArrayList<>();
              for (org.apache.calcite.rel.RelFieldCollation fc : oldCol.getFieldCollations()) {
                int oldIdx = fc.getFieldIndex();
                if (oldIdx < 0 || oldIdx >= projToInput.length) {
                  return visited; // Out of range — bail out.
                }
                newFields.add(fc.withFieldIndex(projToInput[oldIdx]));
              }
              org.apache.calcite.rel.RelCollation newOuterCollation =
                  org.apache.calcite.rel.RelCollations.of(newFields);
              // New Project sits over inner Sort's input; new inner Sort sits over Project; new
              // outer Sort sits over inner Sort with re-indexed collation.
              org.apache.calcite.rel.logical.LogicalProject newProj =
                  org.apache.calcite.rel.logical.LogicalProject.create(
                      innerSort.getInput(),
                      proj.getHints(),
              ...
              org.apache.calcite.rel.logical.LogicalSort newOuter =
                  org.apache.calcite.rel.logical.LogicalSort.create(
                      newInner, newOuterCollation, outerSort.offset, outerSort.fetch);
              return newOuter;
            }
            return visited;
          }
  ...
  /**
   * Add {@code $cor0} to the variablesSet of LogicalProject and LogicalFilter nodes whose
   * expressions contain a RexSubQuery. v2 always tags the parent with $cor0 even for uncorrelated
   * subqueries; the SqlNode→RelNode converter only sets variablesSet for actually-correlated cases.
   * Match v2's emission shape.
   */
  private RelNode addCor0ToSubQueryParents(RelNode rel) {
    final org.apache.calcite.rel.core.CorrelationId cor0 =
        new org.apache.calcite.rel.core.CorrelationId(0);
    org.apache.calcite.rel.RelShuttle shuttle =
        new org.apache.calcite.rel.RelHomogeneousShuttle() {
          @Override
          public RelNode visit(RelNode other) {
            ...
            if (visited instanceof org.apache.calcite.rel.logical.LogicalProject proj) {
              boolean hasSubQuery = containsSubQuery(proj.getProjects());
              if (hasSubQuery && !proj.getVariablesSet().contains(cor0)) {
                java.util.Set<org.apache.calcite.rel.core.CorrelationId> newVars =
                    new java.util.HashSet<>(proj.getVariablesSet());
                newVars.add(cor0);
              ...
              boolean hasSubQuery = containsSubQuery(java.util.List.of(filter.getCondition()));
              if (hasSubQuery && !filter.getVariablesSet().contains(cor0)) {
                java.util.Set<org.apache.calcite.rel.core.CorrelationId> newVars =
                    new java.util.HashSet<>(filter.getVariablesSet());
                newVars.add(cor0);
                return org.apache.calcite.rel.logical.LogicalFilter.create(
                    filter.getInput(),
                    filter.getCondition(),
                    com.google.common.collect.ImmutableSet.copyOf(newVars));
              }
            }
            return visited;
          }
  ...
  private static boolean containsSubQuery(java.util.List<org.apache.calcite.rex.RexNode> exprs) {
    boolean[] found = new boolean[1];
    org.apache.calcite.rex.RexVisitor<Void> visitor =
  ...
  /**
   * Rename auto-named `$fN` columns in pre-aggregate {@link
   * org.apache.calcite.rel.logical.LogicalProject} (the input of a {@link
   * org.apache.calcite.rel.logical.LogicalAggregate}) when the column expression is a {@code SPAN}
   * UDF call. v2's RelBuilder emits the column with user-visible alias {@code span(field,unit)};
   * Calcite's SqlToRelConverter assigns `$fN` because the SPAN call has no explicit alias in GROUP
   * BY context. Mirror v2 by renaming after conversion.
   */
  /**
    ...
    org.apache.calcite.rel.RelShuttle shuttle =
        new org.apache.calcite.rel.RelHomogeneousShuttle() {
          ...
          public RelNode visit(RelNode other) {
            ...
            // For each agg-call output position, collect the names that the Project uses to
            // reference it. If multiple distinct names AND one matches the canonical
            // `<funcName>(...)` pattern, prefer that.
            ...
            // Build new field names for the Aggregate's output row type.
            ...
            for (int i = 0; i < aggFields.size(); i++) {
              newNames.add(aggFields.get(i).getName());
            }
            boolean changed = false;
            for (java.util.Map.Entry<Integer, java.util.LinkedHashSet<String>> entry :
              ...
              if (names.size() < 2) continue; // Only one name — no conflict.
              org.apache.calcite.rel.core.AggregateCall call = agg.getAggCallList().get(aggOutIdx);
              String canonical = canonicalAggName(call);
              // Prefer the canonical name when it appears among the projection names; otherwise
              // pick the FIRST projection name (v2's RelBuilder picks first-alias-wins).
              String preferred;
            ...
            if (!changed) return visited;
            // Rebuild the Aggregate by creating new AggregateCall objects with the desired
            // names. AggregateCall.create accepts a name parameter that becomes the output
            // field name in the row type.
   ...
   * Examples: {@code COUNT()} -> {@code count()}, {@code COUNT($0)} -> {@code count(field)} (if
   * field name available). Returns null when the call's args reference a non-RexInputRef.
   */
  private static String canonicalAggName(org.apache.calcite.rel.core.AggregateCall call) {
    ...
    java.util.List<Integer> argList = call.getArgList();
    if (argList.isEmpty()) {
      return fnName + "()";
    }
    // Multi-arg or non-trivial — skip canonical naming for safety.
    return null;
  }
