...
public abstract class AbstractRexCallTypeCoercion implements RexCallTypeCoercion {

  protected final RexCallValidatorImpl validator;
  protected final RelDataTypeFactory factory;
  protected final RexBuilder rexBuilder;
  protected final SqlNameMatcher nameMatcher = SqlNameMatchers.withCaseSensitive(false);

  // ~ Constructors -----------------------------------------------------------

  AbstractRexCallTypeCoercion(
   ...
   */
  protected boolean coerceOperandType(RexCall call, int index, RelDataType targetType) {
    ...
    RexNode operand = call.operands.get(index);

    RelDataType operandType = validator.deriveType(operand);
    if (coerceStringToArray(call, operand, index, operandType, targetType)) {
      return true;
    }

    // Check it early.
    if (!needToCast(operand, targetType, SqlTypeCoercionRule.lenientInstance())) {
      return false;
    }
    // Fix up nullable attr.
    RelDataType targetType1 = syncAttributes(operandType, targetType);
    RexNode desired = castTo(operand, targetType1);
    call.operands.set(index, desired);
   ...
   */
  protected boolean coerceOperandsType(RexCall call, RelDataType commonType) {
    boolean coerced = false;
    for (int i = 0; i < call.operandCount(); i++) {
      coerced = coerceOperandType(call, i, commonType) || coerced;
    }
    return coerced;
  }
   ...
   */
  protected boolean coerceColumnType(List<RexNode> nodeList, int index, RelDataType targetType) {
    // Transform the JavaType to SQL type because the SqlDataTypeSpec
    // does not support deriving JavaType yet.
    if (RelDataTypeFactoryImpl.isJavaType(targetType)) {
      targetType = ((JavaTypeFactory) factory).toSql(targetType);
    ...
    final RexNode node = nodeList.get(index);
    ...
    if (node instanceof RexCall) {
      RexCall node2 = (RexCall) node;
      if (node2.getOperator().kind == SqlKind.AS) {
        final RexNode operand = node2.operands.get(0);
        if (!needToCast(operand, targetType)) {
          return false;
        }
        RelDataType targetType2 = syncAttributes(validator.deriveType(operand), targetType);
        final RexNode casted = castTo(operand, targetType2);
        node2.operands.set(0, casted);
        updateInferredType(casted, targetType2);
        return true;
      }
    }
    if (!needToCast(node, targetType)) {
      return false;
    }
    RelDataType targetType3 = syncAttributes(validator.deriveType(node), targetType);
    RexNode node3 = castTo(node, targetType3);
    // Preserve the original column name as an alias only if 'node' is an item in a select list.
    //    boolean isSelectItem =
    //        (scope instanceof SelectScope)
  ...
  /**
   * Returns whether a RexNode should be cast to a target type.
   *
   * <p>The default implementation uses the scope's validator's type mapping rule to determine
   * validity; a derived class can override this strategy.
   */
  protected boolean needToCast(RexNode node, RelDataType toType) {
    return needToCast(
        node,
        toType,
        false ? SqlTypeCoercionRule.lenientInstance() : SqlTypeCoercionRule.instance());
  }

  /**
   * Returns whether a RexNode should be cast to a target type, using a type mapping rule to
   * determine casting validity.
   */
  protected boolean needToCast(RexNode node, RelDataType toType, SqlTypeMappingRule mappingRule) {
    RelDataType fromType = validator.deriveType(node);
  ...
  /**
   * It should not be used directly, because some other work should be done before cast operation,
   * see {@link #coerceColumnType}, {@link #coerceOperandType}.
   *
   * <p>Ignore constant reduction which should happen in RexSimplify.
   */
  private RexNode castTo(RexNode node, RelDataType type) {
    return rexBuilder.makeCast(type, node, true, true);
  }

  /** Update inferred type for a RexNode. */
  protected void updateInferredType(RexNode node, RelDataType type) {
    // TODO, do nothing.
  }

  /**
   * Update inferred row type for a query, i.e. RexCall that returns struct type or SqlSelect.
   *
   * @param query Node to inferred type
   * @param columnIndex Column index to update
   * @param desiredType Desired column type
   */
  protected void updateInferredColumnType(RexNode query, int columnIndex, RelDataType desiredType) {
    final RelDataType rowType = validator.deriveType(query);
  ...
  public @Nullable RelDataType getTightestCommonType(
    ...
    if (type1.isStruct() && type2.isStruct()) {
      if (SqlTypeUtil.equalAsStructSansNullability(factory, type1, type2, nameMatcher)) {
  ...
  /**
   * Determines common type for a comparison operator. For date and timestamp operands, use
   * timestamp as common type, i.e. Timestamp(2017-01-01 00:00 ...) &gt; Date(2018) evaluates to be
   * false.
   */
  @Override
  public @Nullable RelDataType commonTypeForBinaryComparison(
    ...
    if (SqlTypeUtil.isString(type1) && SqlTypeUtil.isString(type2)) {
      ...
      if (type1.getCharset() != null
          && type2.getCharset() != null
          && type1.getCharset() != type2.getCharset()) {
        // Cannot coerce; we count on the validator to throw later
        return null;
      }
      if (type1.getCollation() != null
          && type2.getCollation() != null
      ...
      if (type1Name == SqlTypeName.VARCHAR || type2Name == SqlTypeName.VARCHAR) {
        ...
        if (type1Name == SqlTypeName.BINARY
            || type2Name == SqlTypeName.BINARY
            || type1Name == SqlTypeName.VARBINARY
            || type2Name == SqlTypeName.VARBINARY) {
          resultName = SqlTypeName.VARCHAR;
   ...
   * some precision loss when widening decimal to fractional, or promote fractional to string type.
   */
  @Override
  ...
  /**
   * Finds a wider type when one or both types are decimal type. If the wider decimal type's
   * precision/scale exceeds system limitation, this rule will truncate the decimal type to the max
   * precision/scale. For decimal and fractional types, returns a decimal type which has the higher
   ...
   */
  @Override
  public @Nullable RelDataType getWiderTypeForDecimal(
      @Nullable RelDataType type1, @Nullable RelDataType type2) {
    if (type1 == null || type2 == null) {
      return null;
    }
   ...
   * op (b op c). This is only a problem for STRING type or nested STRING type in collection type
   * like ARRAY. Excluding these types, {@link #getWiderTypeForTwo} satisfies the associative law.
   * For instance, (DATE, INTEGER, VARCHAR) should have VARCHAR as the wider common type.
   */
  @Override
  public @Nullable RelDataType getWiderTypeFor(
      List<RelDataType> typeList, boolean stringPromotion) {
  ...
  /**
   * Checks if the types and families can have implicit type coercion. We will check the type one by
   * one, that means the 1th type and 1th family, 2th type and 2th family, and the like.
   *
   ...
   *
   * <p>Caution that we do not cast from NUMERIC to NUMERIC. See <a
  ...
  public @Nullable RelDataType implicitCast(RelDataType in, SqlTypeFamily expected) {
    ...
    if (SqlTypeUtil.isAtomic(in)
        && (expected == SqlTypeFamily.STRING || expected == SqlTypeFamily.CHARACTER)) {
      return expected.getDefaultConcreteType(factory);
    }
    // CHAR -> GEOMETRY
  ...
  protected Boolean coerceStringToArray(
      RexCall call, RexNode operand, int index, RelDataType fromType, RelDataType targetType) {
    return false;
    //    if ( true
    ...
    //        call.operands.set(index, arrayValue);
