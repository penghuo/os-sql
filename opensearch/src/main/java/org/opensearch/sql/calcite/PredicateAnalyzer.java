...
import static org.opensearch.sql.calcite.QueryBuilders.boolQuery;
import static org.opensearch.sql.calcite.QueryBuilders.existsQuery;
import static org.opensearch.sql.calcite.QueryBuilders.rangeQuery;
import static org.opensearch.sql.calcite.QueryBuilders.regexpQuery;
import static org.opensearch.sql.calcite.QueryBuilders.termQuery;
import static org.opensearch.sql.calcite.QueryBuilders.termsQuery;
...
import org.opensearch.sql.calcite.QueryBuilders.BoolQueryBuilder;
import org.opensearch.sql.calcite.QueryBuilders.QueryBuilder;
import org.opensearch.sql.calcite.QueryBuilders.RangeQueryBuilder;
...
class PredicateAnalyzer {
   ...
   * expression cannot be processed (or converted into an OpenSearch query).
   */
   ...
   * an equivalent OpenSearch query filter. If an error occurs, or if it
  ...
  private static class Visitor extends RexVisitorImpl<Expression> {
    ...
    private QueryExpression postfix(RexCall call) {
      ...
      isColumn(a, call, OpenSearchConstants.ID, true);
      isColumn(a, call, OpenSearchConstants.INDEX, true);
    ...
    private QueryExpression binary(RexCall call) {
      ...
      if (isColumn(pair.getKey(), call, OpenSearchConstants.ID, false)
          || isColumn(pair.getKey(), call, OpenSearchConstants.INDEX, false)
          || isColumn(pair.getKey(), call, OpenSearchConstants.UID, false)) {
        switch (call.getKind()) {
        case EQUALS:
        case NOT_EQUALS:
          break;
  ...
  static final class NamedFieldExpression implements TerminalExpression {
    ...
    boolean isMetaField() {
      return OpenSearchConstants.META_COLUMNS.contains(getRootName());
    }
