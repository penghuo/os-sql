...
| 96 | CalciteExplainIT | ✅ | 253/257 pass plus 4 deterministically skipped under no-pushdown. After Track C7 + B5 fixes (which changed the explain-output formatting to include `:VARCHAR` annotations in MAP keys and changed the patterns brain CAST shape), `-Dregen.expected=true` was rerun to refresh 16 affected YAML/JSON files. Earlier wins: F10 bulk regen (485 files), `dropHighlightIfNotRequested` calls in visitAddColTotals/visitAddTotals/visitAppendPipe, PPL_TIMESTAMPDIFF/PPL_TIMESTAMPADD assertion updates. The previously-flaky `testDedupExpr` / `testDedupWithExpr` (HashSet<ScriptField> iteration order) are tolerated via dual-expected primary+alternative match in `MatcherUtils.assertYamlEqualsIgnoreId`. |
