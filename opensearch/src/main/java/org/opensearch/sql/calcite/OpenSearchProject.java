 ...
 * relational expression in OpenSearch.
 */
public class OpenSearchProject extends Project implements OpenSearchRel {
  OpenSearchProject(RelOptCluster cluster, RelTraitSet traitSet, RelNode input,
  ...
  @Override public Project copy(RelTraitSet relTraitSet, RelNode input, List<RexNode> projects,
    ...
    return new OpenSearchProject(getCluster(), traitSet, input, projects, relDataType);
  }
  ...
  @Override public void implement(Implementor implementor) {
    ...
    final List<String> inFields =
            OpenSearchRules.OpenSearchFieldNames(getInput().getRowType());
    final OpenSearchRules.RexToOpenSearchTranslator translator =
            new OpenSearchRules.RexToOpenSearchTranslator(
    ...
    for (Pair<RexNode, String> pair : getNamedProjects()) {
      ...
      // "select *" present?
      hasSelectStar |= OpenSearchConstants.isSelectAll(name);

      if (OpenSearchRules.isItem(pair.left)) {
        implementor.addExpressionItemMapping(name, expr);
        fields.add(expr);
      } else if (expr.equals(name)) {
        fields.add(name);
      } else if (expr.matches("\"literal\":.+")) {
        scriptFields.add(OpenSearchRules.quote(name)
                + ":{\"script\": "
                + expr.split(":")[1] + "}");
      } else {
        scriptFields.add(OpenSearchRules.quote(name)
                ...
                + "\"" + implementor.openSearchTable.scriptedFieldPrefix() + "."
                + expr.replace("\"", "") + "\"}");
      }
    }
    ...
    if (scriptFields.isEmpty()) {
      List<String> newList = fields.stream()
          ...
          .filter(f -> !OpenSearchConstants.ID.equals(f))
          .map(OpenSearchRules::quote)
      ...
      for (String field : fields) {
        scriptFields.add(OpenSearchRules.quote(field) + ":{\"script\": "
                // _source (ES2) vs params._source (ES5)
                + "\"" + implementor.openSearchTable.scriptedFieldPrefix() + "."
                + field + "\"}");
      }
