...
| 119 | CalciteStreamstatsCommandIT | ⚠️ | 43/47 pass after `withRemoveSortInSubQuery(false)` (see #38). 4 fails remaining: `testStreamstatsAndSort`, `testMultipleStreamstats`, `testMultipleStreamstatsWithWindow`, `testMultipleStreamstatsWithEval`, `testStreamstatsResetWithNullBucket`. Root cause: visitor emits `RexOver` with no `ORDER BY` in the `OVER` clause but relies on input row order via a sibling `LogicalSort` or `__stream_seq__=ROW_NUMBER() OVER ()`. ANSI SQL evaluates window aggregates BEFORE `ORDER BY`, so the unparsed SQL (e.g. `SELECT SUM(SAL) OVER (ROWS 1 PRECEDING) FROM emp ORDER BY SAL`) makes the window operate on unsorted scan order. The reset path additionally drifts `Correlate` join type from LEFT to INNER on round-trip, dropping null-bucket rows. Fix requires (a) threading `orderKeys=[__stream_seq__]` through `PlanUtils.makeOver` for all streamstats window emissions, (b) a no-group-path seq column in `visitStreamWindow`, and (c) restoring `Correlate.LEFT` after revalidate. Deferred — full fix tracked under E9. |
