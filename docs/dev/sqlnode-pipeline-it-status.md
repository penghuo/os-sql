...
| 31 | CalcitePPLInSubqueryIT | ⚠️ | 11-17/18 pass (highly flaky, varies 1-11 fails per run; 1 deterministic skip under no-pushdown). Root cause: same Map-to-Comparable codegen issue as ExistsSubqueryIT — correlated IN-subquery's outer scan and inner scans carry `_highlight` (MAP<VARCHAR, ANY>) which leaks into Calcite's enumerable codegen comparators (correlation row materialisation, group-key extraction, sort comparators). Track K17 (`visitFilter` dropHighlight before correlation variable) helps the outer-scan side; tried a scan-level inner strip too but it breaks Join-shaped subqueries by shifting column indices past `_highlight`. Deferred — needs a more targeted fix. |
| 32 | CalcitePPLExistsSubqueryIT | ⚠️ | 18/19 pass after Track K17 (visitor strip in `CalciteRexNodeVisitor.resolveSubqueryPlan` for EXISTS subquery's top-of-stack `_highlight`) + `visitFilter` dropHighlight (drop `_highlight` from outer relbuilder before creating correlation variable). EXISTS only checks row existence — dropping `_highlight` is semantically transparent. IN/Scalar subqueries are NOT touched at the top (they reference specific value columns and any column reordering would break match semantics). Together these stabilise the result to 1 deterministic fail (was 1-6 flaky pre-fix). 1 remaining fail (`testSubsearchMaxOut1`) — pre-existing pre-C6 issue, separate root cause. |
