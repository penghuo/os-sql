  ...
  */
  ...
  public void test_all_params() throws IOException {
    String query =
        "SELECT Id FROM "
            + TEST_INDEX_BEER
            + " WHERE simple_query_string(['Body', Tags, Title], 'taste beer',"
            + " default_operator='or',analyzer=english, analyze_wildcard = false,"
            + " quote_field_suffix = '.exact',auto_generate_synonyms_phrase_query=true, boost ="
            + " 0.77, flags='PREFIX',fuzzy_transpositions = false, lenient = true,"
  ...
  public void verify_wildcard_test() throws IOException {
    ...
    String query2 =
        "SELECT Id FROM " + TEST_INDEX_BEER + " WHERE simple_query_string(['T*'], 'taste')";
