...
public class SessionStateStore {
  ...
  public SessionModel updateState(SessionModel session, SessionState state) {
    try {
      UpdateRequest updateRequest =
          new UpdateRequest()
              ...
              .setIfSeqNo(session.getSeqNo())
              .setIfPrimaryTerm(session.getPrimaryTerm())
              .doc(session.toXContent(XContentFactory.jsonBuilder(), ToXContent.EMPTY_PARAMS))
              .fetchSource(true)
              .setRefreshPolicy(WriteRequest.RefreshPolicy.WAIT_UNTIL);
      ...
      if (updateResponse.getResult().equals(DocWriteResponse.Result.UPDATED)) {
        LOG.debug("Successfully update doc. id: {}", session.getSessionID());
        return SessionModel.of(
            session, state, updateResponse.getSeqNo(), updateResponse.getPrimaryTerm());
      } else {
        throw new RuntimeException(
            String.format(
                Locale.ROOT,
