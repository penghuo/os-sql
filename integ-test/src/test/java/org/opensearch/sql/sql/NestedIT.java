...
public class NestedIT extends SQLIntegTestCase {
  ...
  public void nested_function_with_array_of_nested_field_test() {
    String query =
        "SELECT nested(message.info), nested(comment.data) FROM " + TEST_INDEX_NESTED_TYPE;
  ...
  public void nested_function_in_select_test() {
    String query =
        "SELECT nested(message.info), nested(comment.data), "
  ...
  public void nested_function_in_a_function_in_select_test() {
    String query =
        "SELECT upper(nested(message.info)) FROM " + TEST_INDEX_NESTED_TYPE_WITHOUT_ARRAYS;
    JSONObject result = executeJdbcRequest(query);

    verifyDataRows(result, rows("A"), rows("B"), rows("C"), rows("C"), rows("ZZ"));
  }
  ...
  public void invalid_multiple_nested_all_function_in_a_function_in_select_test() {
    String query =
        "SELECT nested(message.*), nested(message.info) FROM "
            + TEST_INDEX_NESTED_TYPE_WITHOUT_ARRAYS;
    RuntimeException result = assertThrows(RuntimeException.class, () -> executeJdbcRequest(query));
    assertTrue(
  ...
  public void nested_function_with_array_of_multi_nested_field_test() {
    ...
    assertEquals(6, result.getInt("total"));
    verifyDataRows(result, rows("e"), rows("f"), rows("g"), rows("h"), rows("p"), rows("yy"));
  }

  @Test
  public void nested_function_with_null_and_missing_fields_test() {
    String query =
        "SELECT nested(message.info), nested(comment.data) FROM " + TEST_INDEX_NESTED_WITH_NULLS;
    JSONObject result = executeJdbcRequest(query);
  ...
  public void nested_function_multiple_fields_with_matched_and_mismatched_paths_test() {
    String query =
        "SELECT nested(message.author), nested(message.dayOfWeek), nested(message.info),"
            + " nested(comment.data), nested(comment.likes) FROM "
  ...
  public void nested_function_mixed_with_non_nested_type_test() {
    String query = "SELECT nested(message.info), someField FROM " + TEST_INDEX_NESTED_TYPE;
    JSONObject result = executeJdbcRequest(query);
  ...
  public void nested_function_with_order_by_clause() {
    String query =
        "SELECT nested(message.info) FROM "
            + TEST_INDEX_NESTED_TYPE
            + " ORDER BY nested(message.info)";
    JSONObject result = executeJdbcRequest(query);

    assertEquals(6, result.getInt("total"));
    verifyDataRows(result, rows("a"), rows("c"), rows("a"), rows("b"), rows("c"), rows("zz"));
  }
  ...
  public void nested_function_with_order_by_clause_desc() {
    ...
    assertEquals(6, result.getInt("total"));
    verifyDataRows(result, rows("zz"), rows("c"), rows("c"), rows("a"), rows("b"), rows("a"));
  }

  @Test
  public void nested_function_and_field_with_order_by_clause() {
    String query =
        "SELECT nested(message.info), myNum FROM "
            + TEST_INDEX_NESTED_TYPE
  ...
  public void nested_function_with_group_by_clause() {
    ...
    assertTrue(
        result
            ...
            .get("details")
            .toString()
            .contains("Aggregation type nested is not yet implemented"));
  }
  ...
  public void nested_function_with_having_clause() {
    String query =
        "SELECT count(*) FROM "
            + TEST_INDEX_NESTED_TYPE
            + " GROUP BY myNum HAVING nested(comment.likes) > 7";
    JSONObject result = executeJdbcRequest(query);
  ...
  public void nested_function_mixed_with_non_nested_types_test() {
    ...
    verifyDataRows(
        result,
        rows("a", new JSONObject(Map.of("south", 3, "west", "ab")), "ab"),
        rows("b", new JSONObject(Map.of("south", 5, "west", "ff")), "ff"),
        rows("c", new JSONObject(Map.of("south", 3, "west", "ll")), "ll"),
        rows("d", null, null),
        rows("i", null, null),
        rows("zz", null, null));
  }
        ...
        exception
            ...
            .contains(
                "{\n"
                    + "  \"error\": {\n"
                    + "    \"reason\": \"Invalid SQL query\",\n"
                    + "    \"details\": \"can't resolve Symbol(namespace=FIELD_NAME,"
                    + " name=message.invalid) in type env\",\n"
        ...
        exception
            ...
            .contains(
                "{\n"
                    + "  \"error\": {\n"
                    + "    \"reason\": \"Invalid SQL query\",\n"
                    + "    \"details\": \"can't resolve Symbol(namespace=FIELD_NAME, name=invalid)"
                    + " in type env\",\n"
                    + "    \"type\": \"SemanticCheckException\"\n"
                    + "  },\n"
                    + "  \"status\": 400\n"
  ...
  public void test_nested_in_where_as_predicate_expression() {
    String query =
        "SELECT message.info FROM " + TEST_INDEX_NESTED_TYPE + " WHERE nested(message.info) = 'a'";
    JSONObject result = executeJdbcRequest(query);
  ...
  public void test_nested_in_where_as_predicate_expression_with_multiple_conditions() {
    String query =
        "SELECT message.info, comment.data, message.dayOfWeek FROM "
            + TEST_INDEX_NESTED_TYPE
            + " WHERE nested(message.info) = 'zz' OR nested(comment.data) = 'ab' AND"
  ...
  public void test_nested_in_where_as_predicate_expression_with_relevance_query() {
    String query =
        "SELECT comment.likes, someField FROM "
            + TEST_INDEX_NESTED_TYPE
  ...
  public void nested_function_all_subfields_and_specified_subfield() {
    String query = "SELECT nested(message.*), nested(comment.data) FROM " + TEST_INDEX_NESTED_TYPE;
    JSONObject result = executeJdbcRequest(query);
  ...
  public void nested_function_all_deep_nested_subfields() {
    String query = "SELECT nested(message.author.address.*) FROM " + TEST_INDEX_MULTI_NESTED_TYPE;
    JSONObject result = executeJdbcRequest(query);
  ...
  public void nested_function_all_subfields_for_two_nested_fields() {
    String query = "SELECT nested(message.*), nested(comment.*) FROM " + TEST_INDEX_NESTED_TYPE;
    JSONObject result = executeJdbcRequest(query);
  ...
  public void nested_function_with_date_types_as_object_arrays_within_arrays_test() {
    ...
    verifySchema(result, schema("nested(address.moveInDate)", null, "object"));
    verifyDataRows(
        result,
