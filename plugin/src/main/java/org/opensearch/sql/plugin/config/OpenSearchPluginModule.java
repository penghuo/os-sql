...
import org.opensearch.sql.opensearch.dqe.DistributedExecutor;
import org.opensearch.sql.opensearch.dqe.ShardRoutingResolver;
...
import org.opensearch.sql.plugin.transport.CalciteShardQueryDispatcher;
...
public class OpenSearchPluginModule extends AbstractModule {
  ...
  public ExecutionEngine executionEngine(
    ...
    Boolean dqeEnabled = settings.getSettingValue(Settings.Key.DQE_ENABLED);
    if (Boolean.TRUE.equals(dqeEnabled)) {
      ShardRoutingResolver resolver = new ShardRoutingResolver(clusterService);
      CalciteShardQueryDispatcher dispatcher = new CalciteShardQueryDispatcher(nodeClient);
      DistributedExecutor executor = new DistributedExecutor(resolver, dispatcher);
      engine.setDistributedExecutor(executor);
    }
