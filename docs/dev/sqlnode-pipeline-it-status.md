...
| # | Class | Pushdown ON | Notes |
|---|---|---|---|
| 80 | CalciteJsonFunctionsIT | ⏭️ | Track X30 closed 2 of 5 failures: introduced `JsonValidFunctionImpl` (UDF in `core/.../jsonUDF/`) registered for `JSON_VALID` in `PPLBuiltinOperators` and `PPLFuncImpTable` so Calcite's `json_valid` mirrors v2's Jackson-based contract (empty string → valid, null → false). `test_json_valid` and `test_not_json_valid` now pass. Three remaining failures (`test_cast_json`, `test_json`, `test_cast_json_scalar_to_type`) need `cast(x as json)` and `json(x)` to return the parsed-and-re-serialized canonical form (no whitespace) instead of the raw string — feature work tracked under issue #3436. Class re-`@Ignore`'d to keep CI green. |
