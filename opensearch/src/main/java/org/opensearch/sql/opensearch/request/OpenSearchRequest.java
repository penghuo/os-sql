...
public interface OpenSearchRequest extends Writeable {
  ...
  /** Whether incremental reduce output can be parsed into source-level PPL rows. */
  default boolean supportsAggregationSnapshots() {
    return false;
  }

  /** Parses one incremental reduce output without applying Calcite root result semantics. */
