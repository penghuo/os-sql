...
| # | Class | Pushdown ON | Notes |
|---|---|---|---|
| 80 | CalciteJsonFunctionsIT | ✅ | All 5/5 pass after Tracks X30, Y31, and Z32: X30's `JsonValidFunctionImpl` mirrors v2's `json_valid` contract (empty string → valid, null → false). Y31 updates `JsonFunctionImpl.eval` to parse via Jackson and re-serialize via Gson canonical form. Z32: (a) `CalciteRexNodeVisitor.visitCast` routes `cast(x as json)` through `PPLBuiltinOperators.JSON` instead of an identity-cast to STRING (Cast.getDataType collapses "json" to "string" since Calcite has no first-class JSON type); (b) `MatcherUtils.rows` falls back to a JSON-aware similar comparator that allows: `String JSON` vs `JSONObject`/`JSONArray` (parse-and-compare), `String` vs `Number`/`Boolean` (parse-and-compare with float-precision tolerance), and `String JSON` vs `String JSON` (parse both, content-compare regardless of field order). |
...
| 113 | CalcitePrometheusDataSourceCommandsIT | ✅ | All 11 pass after Track W29: `QueryService.executeWithLegacy` now prefers the v2 fallback's error message (instead of the Calcite original) when the v2 error is a `DataSourceClientException` (e.g. `DatasourceDisabledException`, `DataSourceNotFoundException`) AND the Calcite error is the generic `CalciteUnsupportedException`. This restores the v2 error contract (`reason: "Invalid Query"`, `details: "Datasource X is disabled."`) for disabled/missing data sources while preserving the existing #5060 behaviour for all other cases. |
